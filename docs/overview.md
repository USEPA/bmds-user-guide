...
EPA's preferred approach for dose-response modeling is [Benchmark Dose modeling](https://www.epa.gov/risk/benchmark-dose-technical-guidance).
...
A dichotomous dose-response relationship modeled with the Dichotomous Hill model
```
As {numref}`f1` shows, the benchmark dose (BMD) is the dose estimated to produce a benchmark response (BMR) of a specific magnitude in the test subject. The [online BMDS Glossary](https://sor.epa.gov/sor_internet/registry/termreg/searchandretrieve/termsandacronyms/search.do) defines the BMD as follows:
...
The BMD can be thought of as a range, rather than a fixed number, and is considered in the context of its confidence interval. The BMD's lower confidence limit (BMDL) is a dose where the observable physical effect is *less than* the predetermined BMR. The BMD's upper confidence limit (BMDU) is a dose where the observable effect is *greater than* the predetermined BMR.

Collectively, the application of the methods for fitting mathematical models to data is referred to as BMD modeling or the BMD approach. The EPA Risk Assessment Forum has published [technical guidance](https://www.epa.gov/risk/benchmark-dose-technical-guidance) for the application of the BMD approach in cancer and non-cancer dose-response assessments ([U.S. EPA, 2012](https://www.epa.gov/risk/benchmark-dose-technical-guidance?action=search.view&reference_id=1239433)). BMDS facilitates these operations by providing simple data-management tools and an easy-to-use interface to run multiple models on one or more dose-response datasets.
...
-   [**Continuous Endpoints**](./continuous-mle.md): the response is measured on a continuous scale, so its valid values are real numbers (often restricted to positive values, but not always). Organ weight, body weight, and concentration levels of biological markers are all examples of continuous endpoints whose values might be affected by the exposure under consideration.

-   [**Dichotomous Endpoints**](./dichotomous-mle.md): the response of interest is the presence or absence of a disease state or other indication of organ system or tissue malfunction. In this case, each experimental unit will either have the response or not. Dichotomous dose-response models predict the proportion of such units that have the response. Tumor responses are a subcategory of dichotomous endpoints with cancer-specific model selection considerations that are automated in the BMDS Multitumor model; for more information, see [**Multiple Tumor Analysis**](./multiple-tumor-analysis).

-   [**Nested Dichotomous Endpoints**](./nested-dichotomous.md): as for dichotomous endpoints, the presence or absence of a disease state is the response. However, in this case each experimental unit may have more than one such count (*i.e.*, the counts are nested within an experimental unit). The most common example of such an endpoint is with developmental toxicity experiments in which the experimental unit is the pregnant dam and the fetuses or offspring from each dam's litter are examined for the presence or absence of an effect (*e.g.*, malformation).

### Presentation of Model Results

As {numref}`f2` shows, BMDS presents [**model results**](./result-output-mle-bayesian.md) in text and graphic outputs that can be printed or saved and incorporated into other documents. Results from all models include:
...
-   BMDL and BMDU, estimates of the lower and upper bounds (*e.g.*, confidence limits) on the BMD, respectively
...
-   It is limited to a single tested dose in the study rather than using all the available dose-response information.

-   The identified NOAEL or LOAEL is highly dependent on study design, including sample size.

-   It does not account for variability in the estimate of the dose-response.

-   It does not account for the slope of the dose-response curve, including non-monotonic dose-response relationships.

-   Use of the LOAEL requires the application of an uncertainty factor ([Kimmel and Gaylor, 1988](https://hero.epa.gov/hero/index.cfm?action=search.view&reference_id=4250); [Crump, 1984](https://hero.epa.gov/hero/index.cfm?action=search.view&reference_id=66651)).

Overall, using the BMD approach allows EPA to define a point of departure (POD) for the computation of a reference value (RfV) or cancer slope factor (CFS) that is more independent of study design and is not limited by the factors noted above. 
